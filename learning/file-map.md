...
| `backend/main.py` | Archivo Python | known | Punto de entrada de FastAPI; carga y valida `DATABASE_URL` desde `backend/.env`, permite CORS solo para el frontend local y expone `GET /health`, `GET /health/database`, `POST /jobs` y `GET /jobs/{job_id}`. La ruta POST valida el nombre de un archivo, crea un `JobRecord` mediante SQLAlchemy y lo confirma en PostgreSQL; la GET recupera el trabajo y sus filas relacionadas o devuelve 404. La ruta de base de datos usa `psycopg` y `SELECT 1` para comprobar una conexión real. |
